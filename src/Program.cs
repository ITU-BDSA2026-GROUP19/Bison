...
CSVDatabase<Observation> observationsDatabase =
    CSVDatabase<Observation>.GetInstance("bison_observations.csv");

CSVDatabase<Comment> commentsDatabase =
    CSVDatabase<Comment>.GetInstance("bison_comments.csv");
...
var observeMessageArg = new Argument<string>("message")
{
    Description = "The observation message"
};
...
observeCommand.SetAction((result) =>
{
    ...
    int nextId = observationsDatabase
        .Read()
...
var commentObservationIdArg = new Argument<int>("observation-id")
{
    Description = "The ID of the observation to comment on"
};

var commentMessageArg = new Argument<string>("message")
{
...
commentCommand.SetAction((result) =>
{
    ...
    bool observationExists = observationsDatabase
        .Read()
        .Any(observation => observation.Id == observationId);
...
var discussionObservationIdArg = new Argument<int>("observation-id")
{
    Description = "The ID of the observation"
};

var discussionCommand = new Command(
    "discussion",
    "Read comments for an observation"
);
...
discussionCommand.SetAction((result) =>
{
    ...
    IEnumerable<Comment> comments = commentsDatabase
        .Read()
...
var rootCommand = new RootCommand(
    "Bison CLI - observe, read, comment and discuss"
);
